      ...
      <div class="checkout-tit">
        ...
        <div class="paymark">
          <span class="fl">请您在提交订单<em
              class="orange time">4小时</em>之内完成支付，超时订单会自动取消。订单号：<em>{{ orderInfo.orderId }}</em></span>
...
export default {
  ...
  methods: {
    async getPayInfo() {
      let res = await this.$api.getPayInfo(this.$route.query.orderData)
      if (res.code == 200) {
    ...
    async open() {
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          let res = await this.$api.getPayState(this.orderInfo.orderId)
          if (res.code == 200) {
            clearInterval(this.timer)
            this.timer = null
            this.code = res.code
