        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="s in skuSaleAttrValueList" :key="s.id">
                <dt class="title">{{s.saleAttrName}}</dt>
                <dd changepirce="0" class="active">{{s.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model.lazy.trim.number="userInput" @change="strictNum">
                <a href="javascript:" class="plus" @click="userInput>98? 99: userInput++">+</a>
                <a href="javascript:" class="mins"  @click="userInput>1 ? userInput-- : 1">-</a>
              </div>
              <div class="add">
...
import { mapGetters } from 'vuex'
  export default {
    ...
    mounted() {
 this.$store.dispatch('detail/getGoodsdetail',this.$route.params.stuid)
  },
  computed: {
  ...
  methods:{
    strictNum(e){
         ...
         this.userInput= this.userInput>98 ? 99 : this.userInput
    },
    async addCart(){
      if(localStorage.getItem('token')) {
        try {
            await this.$store.dispatch('detail/addGoodsToCart',{stuId:this.$route.params.stuid,stuNum:this.userInput})    
            this.$router.push({
              name:'addcartsuccess',
              query:{
